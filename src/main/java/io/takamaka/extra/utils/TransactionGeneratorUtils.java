 ...
 */
@Slf4j
public class TransactionGeneratorUtils {
    ...
    public static String[] generateTransactionArrayWithPay(int numberOfTransaction, KeyContexts.TransactionType type, String walletPrefix, int numberOfFromWallets, int numberOfToWallets, int fromWalletSpace, int toWalletSpace) throws TransactionCanNotBeCreatedException, TransactionNotYetImplementedException, InvalidWalletIndexException, PublicKeySerializzationException, UnlockWalletException {
        String[] transactions = new String[numberOfTransaction * 2];
        //System.out.println("number of transactions: " + transactions.length);
        InstanceWalletKeyStoreBCED25519[] fromKeys = new InstanceWalletKeyStoreBCED25519[numberOfFromWallets];
        InstanceWalletKeyStoreBCED25519[] toKeys = new InstanceWalletKeyStoreBCED25519[numberOfToWallets];
        //System.out.println("From keyspace: " + numberOfFromWallets * fromWalletSpace);
...
//            }
    ...
    public static String[] generateTransactionArrayWithPresonalizedValues(InstanceWalletKeystoreInterface[] wallets, Integer[] keyIndex, KeyContexts.TransactionType types[], String[] from, String[] to, BigInteger[] greenValues, BigInteger[] redValues, String[] messages) throws TransactionNotYetImplementedException, TransactionCanNotBeCreatedException {
        if (!areSameLength(wallets, keyIndex, types, from, to, greenValues, redValues, messages)) {
            System.out.println("Parameters have different lengths!");
            return null;
        } else {
            String[] ret = new String[wallets.length];
            for (int i = 0; i < ret.length; i++) {
                ...
                TransactionBean tb = TkmWallet.createGenericTransaction(itb, wallets[i], keyIndex[i]);
                ret[i] = TkmTextUtils.toJson(tb);
            }
            return ret;
        }
    }

    public static final String[] generateTransactions(int numberOfTransactions, KeyContexts.TransactionType type, KeyContexts.WalletCypher wc, String walletPrefix, int numberOfWallets, int fromIndexesKeyspace, int toIndexesKeyspace, String greenValPattern, String redValPattern) {
        String[] res = new String[numberOfTransactions];
        InstanceWalletKeystoreInterface[] fIwk = new InstanceWalletKeystoreInterface[numberOfWallets];
        InstanceWalletKeystoreInterface[] tIwk = new InstanceWalletKeystoreInterface[numberOfWallets];

        range(0, numberOfWallets).parallel().forEach(i -> {
            try {
                switch (wc) {
                    case BCQTESLA_PS_1:
                        fIwk[i] = new InstanceWalletKeyStoreBCQTESLAPSSC1Round1(walletPrefix + "_from_" + i);
                        tIwk[i] = new InstanceWalletKeyStoreBCQTESLAPSSC1Round1(walletPrefix + "_to_" + i);
                        break;
                    case BCQTESLA_PS_1_R2:
                        fIwk[i] = new InstanceWalletKeyStoreBCQTESLAPSSC1Round2(walletPrefix + "_from_" + i);
                        tIwk[i] = new InstanceWalletKeyStoreBCQTESLAPSSC1Round2(walletPrefix + "_to_" + i);
                        break;
                    case Ed25519BC:
                        fIwk[i] = new InstanceWalletKeyStoreBCED25519(walletPrefix + "_from_" + i);
                        tIwk[i] = new InstanceWalletKeyStoreBCED25519(walletPrefix + "_to_" + i);
                        break;
                    default:
                        throw new RuntimeException(wc.name());
                }
        ...
        System.out.println("");
        range(0, numberOfTransactions).parallel().forEach(i -> {
            Exception e = null;
            do {
                e = null;
                try {
                    int indexOfFromWallet = getPositiveLimitedInt(fromIndexesKeyspace);
                    int indexOfToWallet = getPositiveLimitedInt(toIndexesKeyspace);
                    int numberOfFromWallet = getPositiveLimitedInt(numberOfWallets);
                    int numberOfToWallet = getPositiveLimitedInt(numberOfWallets);
                    int greenVal = getPositiveStringByPatternLength(greenValPattern);
                    int redVal = getPositiveStringByPatternLength(redValPattern);

                    BigInteger greenValBI = new BigInteger("" + greenVal);
                    BigInteger redValBI = new BigInteger("" + redVal);

                    if (greenValBI.equals(BigInteger.ZERO) && redValBI.equals(BigInteger.ZERO)) {
                        greenValBI = new BigInteger("9000000000");
                    }

                    InternalTransactionBean itb = getInternalTransactionBean(
                            type,
                            fIwk[numberOfFromWallet].getPublicKeyAtIndexURL64(indexOfFromWallet),
                            tIwk[numberOfToWallet].getPublicKeyAtIndexURL64(indexOfToWallet),
                            greenValBI,
                            redValBI,
                            fIwk[numberOfFromWallet].getCurrentWalletID() + " " + tIwk[numberOfFromWallet].getCurrentWalletID());

                    TransactionBean genericTR = TkmWallet.createGenericTransaction(
                            itb,
                            fIwk[numberOfFromWallet],
                            indexOfFromWallet);

                    res[i] = TkmTextUtils.toJson(
                            genericTR
                    );
                    if ((i % 500) == 0) {
                        System.out.print(".");
    ...
    public static final int getPositiveStringByPatternLength(String a) {
        int fksl = (a + "").length();
        String fkslString = RandomStringUtils.randomNumeric(1, fksl + 1);
        int fkslInt = Math.abs(Integer.parseInt(fkslString));
        return fkslInt;
    }

    public static final int getPositiveLimitedInt(int limit) {
        boolean inLimit = false;
        int candidate = Integer.MAX_VALUE;
        do {
            candidate = getPositiveStringByPatternLength(limit + "");
            if (candidate < limit) {
                inLimit = true;
            }
        } while (!inLimit);
    ...
    public static String[] generateTransactionArrayWithPayQT(int numberOfTransaction, KeyContexts.TransactionType type, String walletPrefix, int numberOfFromWallets, int numberOfToWallets, int fromWalletSpace, int toWalletSpace) throws TransactionCanNotBeCreatedException, TransactionNotYetImplementedException, InvalidWalletIndexException, PublicKeySerializzationException, UnlockWalletException {
        String[] transactions = new String[numberOfTransaction * 2];
        System.out.println("number of transactions: " + transactions.length);
        InstanceWalletKeystoreInterface[] fromKeys = new InstanceWalletKeyStoreBCQTESLAPSSC1Round1[numberOfFromWallets];
        InstanceWalletKeystoreInterface[] toKeys = new InstanceWalletKeyStoreBCQTESLAPSSC1Round1[numberOfToWallets];
        System.out.println("From keyspace: " + numberOfFromWallets * fromWalletSpace);
        System.out.println("To keyspace: " + numberOfFromWallets * fromWalletSpace);
        IntStream.range(0, fromKeys.length).parallel().forEach(i -> {
            try {
                fromKeys[i] = new InstanceWalletKeyStoreBCQTESLAPSSC1Round1(walletPrefix + "_tkm_test_factory_demo_from_wallet_BCQTESLAPSSC1_" + i);
            } catch (UnlockWalletException ex) {
                log.error(ex.getLocalizedMessage());
            }
        });
         ...
         */
        IntStream.range(0, toKeys.length).parallel().forEach(i -> {
            try {
                toKeys[i] = new InstanceWalletKeyStoreBCQTESLAPSSC1Round1(walletPrefix + "_tkm_test_factory_demo_to_wallet_BCQTESLAPSSC1_" + i);
            } catch (UnlockWalletException ex) {
                log.error(ex.getLocalizedMessage());
            }
        });
        ...
        boolean firstRun = false;
        //for (int i = 0; i < numberOfTransaction; i++) {
        IntStream.range(0, numberOfTransaction).parallel().forEach(i -> {
            /*
                if (i % indPrg == 0) {
                    if (firstRun) {
                        float val = i;
            ...
            try {
                ...
                itb = getInternalTransactionBean(
                        type,
                        fromKeys[fromWallet].getPublicKeyAtIndexURL64(fromKey),
                        toKeys[toWallet].getPublicKeyAtIndexURL64(toKey),
                        pickANumberAsCoin(10),
                ...
                transactions[i + numberOfTransaction] = TkmTextUtils.toJson(
                        TkmWallet.createGenericTransaction(
                                getInternalTransactionBean(
                                        ...
                                        fromKeys[fromWallet].getPublicKeyAtIndexURL64(fromKey),
                                        itb.getGreenValue().divide(BigInteger.ONE.add(BigInteger.ONE)),
                                        itb.getRedValue().divide(BigInteger.ONE.add(BigInteger.ONE)),
                                        createIdentifingMessage(walletPrefix, fromWallet, fromKey, toWallet, toKey) + " PAY"
                                ),
                                fromKeys[fromWallet],
                                fromKey)
                );
            } catch (TransactionNotYetImplementedException | WalletException e) {
                System.out.println("--------------------------------------------------------");
    ...
    public static String[] generateRegisterMainTransactionArrayWithQT(int numberOfTransaction, KeyContexts.TransactionType type, String walletPrefix, int numberOfWallets, int fromWalletSpace) throws TransactionCanNotBeCreatedException, TransactionNotYetImplementedException, InvalidWalletIndexException, PublicKeySerializzationException, UnlockWalletException {
        String[] transactions = new String[numberOfTransaction * 2];
        System.out.println("number of transactions: " + transactions.length);
        InstanceWalletKeystoreInterface[] fromKeys = new InstanceWalletKeyStoreBCQTESLAPSSC1Round1[numberOfWallets];
        System.out.println("From keyspace: " + numberOfWallets * fromWalletSpace);
        for (int i = 0; i < fromKeys.length; i++) {
            fromKeys[i] = new InstanceWalletKeyStoreBCQTESLAPSSC1Round1(walletPrefix + "_tkm_test_factory_demo_from_wallet_BCQTESLAPSSC1_" + i);
        }
        System.out.print("Progress: 0%");
        int indPrg = numberOfTransaction / 2;
        boolean firstRun = false;
        for (int i = 0; i < numberOfTransaction; i++) {
            try {
                if (i % indPrg == 0) {
                ...
                int fromWallet = pickANumberInRange(0, numberOfWallets);
                int fromKey = pickANumberInRange(0, numberOfWallets);
                InternalTransactionBean itb = getInternalTransactionBean(
                        type,
                        fromKeys[fromWallet].getPublicKeyAtIndexURL64(fromKey),
                        StringUtils.EMPTY,
                        BigInteger.ZERO,
                        BigInteger.ZERO,
                        createIdentifingMessage(walletPrefix, fromWallet, fromKey, 0, 0)
                );
                TransactionBean genericTR = TkmWallet.createGenericTransaction(
                        itb,
                        fromKeys[fromWallet],
                ...
                transactions[i + numberOfTransaction] = TkmTextUtils.toJson(
                        TkmWallet.createGenericTransaction(
                                getInternalTransactionBean(
                                        ...
                                        BigInteger.ZERO,
                                        BigInteger.ZERO,
                                        createIdentifingMessage(walletPrefix, fromWallet, fromKey, 0, 0) + " REGISTER_MAIN"
                                ),
                                fromKeys[fromWallet],
                                fromKey)
                );
    ...
    public static String[] generateRegisterMainTransactionArray(int numberOfTransaction, KeyContexts.TransactionType type, String walletPrefix, int numberOfWallets, int fromWalletSpace) throws TransactionCanNotBeCreatedException, TransactionNotYetImplementedException, InvalidWalletIndexException, PublicKeySerializzationException, UnlockWalletException {
        String[] transactions = new String[numberOfTransaction * 2];
        System.out.println("number of transactions: " + transactions.length);
        InstanceWalletKeystoreInterface[] fromKeys = new InstanceWalletKeyStoreBCED25519[numberOfWallets];
        //InstanceWalletKeystoreInterface[] toKeys = new InstanceWalletKeyStoreBCQTESLAPSSC1[numberOfToWallets];
        System.out.println("From keyspace: " + numberOfWallets * fromWalletSpace);
        //System.out.println("To keyspace: " + numberOfFromWallets * fromWalletSpace);
        for (int i = 0; i < fromKeys.length; i++) {
            fromKeys[i] = new InstanceWalletKeyStoreBCED25519(walletPrefix + "_tkm_test_factory_demo_from_wallet_BCED25519_" + i);
        }
//        for (int i = 0; i < toKeys.length; i++) {
//            toKeys[i] = new InstanceWalletKeyStoreBCQTESLAPSSC1(walletPrefix + "_tkm_test_factory_demo_to_wallet_BCQTESLAPSSC1_" + i);
//        }
        System.out.print("Progress: 0%");
        int indPrg = numberOfTransaction / 2;
        boolean firstRun = false;
                ...
                transactions[i + numberOfTransaction] = TkmTextUtils.toJson(
                        TkmWallet.createGenericTransaction(
                                getInternalTransactionBean(
                                        ...
                                        BigInteger.ZERO,
                                        BigInteger.ZERO,
                                        createIdentifingMessage(walletPrefix, fromWallet, fromKey, 0, 0) + " " + type.name()
                                ),
                                fromKeys[fromWallet],
                                fromKey)
                );
...
//        }
        ...
        for (int i = 0; i < numberOfTransaction; i++) {
            try {
                ...
                InternalTransactionBean itbD = getInternalTransactionBean(
                        KeyContexts.TransactionType.DECLARATION,
                        fromKeys[fromWallet].getPublicKeyAtIndexURL64(fromKey),
                        fromKeys[fromWallet].getPublicKeyAtIndexURL64(fromKey),
                        new BigInteger("10000" + i),
                        new BigInteger("10000" + i),
                        createIdentifingMessage(walletPrefix, fromWallet, fromKey, 0, 0)
                );

                TransactionBean genericDTR = TkmWallet.createGenericTransaction(
                        itbD,
                        fromKeys[fromWallet],
                        fromKey);
